## 최신 릴리스 (Swift {{ site.data.builds.swift_releases.last.name }}) {#latest}
...
<ul class="grid-level-0 grid-layout-2-column">
  ...
  <li class="grid-level-1">
    ...
    <ul class="grid-level-0 grid-layout-2-column">
      {% for arch in platform.archs %}
      <a href="https://download.swift.org/{{ tag_downcase }}/{{ platform_name_url }}{% if arch != "x86_64" %}-{{ arch }}{% endif %}/{{ tag }}/{{ tag }}-{{ platform_name }}{% if arch != "x86_64" %}-{{ arch }}{% endif %}.tar.gz" class="cta-secondary">다운로드 ({{ arch }})</a>
      {% endfor %}
    </ul>
    <a href="/install/linux/tarball" class="cta-secondary">안내</a>
  </li>
</ul>
...
<h3>RPM</h3>
<p class="description">
RPM Package Manager (실험적 용도)
</p>
<a href="/install/linux/rpm" class="cta-secondary">안내</a>
</li>
{% endif %}
